...
1. **Calibrate** -- click 8 court corners + 1 net point on the first frame. Computes a homography that maps pixels to court coordinates (feet).
...
3. **Review** -- opens an interactive viewer to step through detected faults and accept/reject each one.
...
```bash
python calibrate.py game.mp4 --court-id home --camera-id gopro1
```
...
Press `u` to undo the last point, `q` to quit. After placing all 9 points, a validation overlay shows the kitchen zones. Press any key to save, or `r` to redo.

Saves to `configs/{court_id}_{camera_id}.json`.

### 2. Detect faults

```bash
python detect.py game.mp4 --calibration configs/home_gopro1.json

# With ball detection model (optional)
python detect.py game.mp4 --calibration configs/home_gopro1.json --ball-model ball_detect.pt

# With a different pose model
python detect.py game.mp4 --calibration configs/home_gopro1.json --pose-model yolov8l-pose.pt
```
...
**Zone check:** Each frame, foot keypoints (or ankle keypoints with a 0.5ft buffer) are transformed through the homography to court coordinates. If any keypoint lands inside a kitchen zone (0-7ft from baseline on either side), it's a zone hit.
